...
| Companion nella chat Desktop | Completato, collaudo fisico riservato all'utente | Il modello Mac sceglie esplicitamente il budget di output 256-16384 token per ogni delega; il Desktop lo limita alla capacita realmente annunciata dall'iPhone e mantiene orchestrazione/fallback. Una generazione che esaurisce il budget o non entra nel contesto viene rifiutata invece di consegnare testo troncato. Auto-best e Multi-iPhone restano attivi |
...
| App/DMG macOS | Completato | Build `2026.8.18+mlxcompaction8.companion9` / `0.1.800-mlx.9`: arm64, macOS 12, metadata, firma ad hoc, privacy rete locale e servizio Bonjour dichiarati, wheel Companion incorporata e `hdiutil verify` |
...
Durante l'intervento originale `companion9` non furono eseguiti nuovi test automatici o collaudi su iPhone, come richiesto allora. Per la pubblicazione open source del 2026-08-30 sono stati però rieseguiti con esito positivo 13 test Python del protocollo/manager Desktop, 13 test Swift unitari e 3 UI test iOS firmati; l'IPA unsigned è stata rigenerata dai sorgenti correnti e ricontrollata. Non è stato eseguito un nuovo task subagente su iPhone fisico, quindi il gate hardware sotto resta invariato.

- 13 test Swift unitari.
- 3 test UI iOS firmati.
- 13 test Python del Companion Desktop, incluso WSS TLS reale in loopback.
